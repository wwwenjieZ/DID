...
data_potential <- data %>%
  ...
  mutate(non_response = ifelse(non_response_prob > 0.07, 1, 0)) %>%
  filter(!(time == 0 & C > 0 & non_response == 1)) %>%
  select(id, time, D, C, Y, d,did,non_response_prob,non_response )
nrow(data_potential)
#non_response_prob > 0.13  8502
#non_response_prob > 0.23  9094
#non_response_prob > 0.35  9489
#non_response_prob > 0.07 8023

sum(data_potential$time == 1)
sum(data_potential$time == 0)
###############################################
...
############################################
#n_simulations=100
#model_results_FE_SSEL_bal <- vector("list", n_simulations)
#for (i in 1:n_simulations) {
#  data <- data.frame(
#    id = rep(1:n, each = 2),   
#    time = rep(0:1, times = n),  
...
#  # Create a balanced sample
#  ids_in_both_periods <- intersect(
#    data_SSEL %>% filter(time == 0) %>% select(id) %>% unlist(),
#    data_SSEL %>% filter(time == 1) %>% select(id) %>% unlist()
#  )
#  balanced_dataAT <-data_SSEL %>% filter(id %in% ids_in_both_periods)
#  
...
#  didFE_SSEL_bal <- plm(Y ~ d + time + did, data = balanced_dataAT_P, model="within")
#  model_results_FE_SSEL_bal[[i]] <- coef(didFE_SSEL_bal)
#}
#
##----results---#
#results_matrix_FE_SSEL_bal <- do.call(rbind, model_results_FE_SSEL_bal)
#mean_coeffs_FE_SSEL_bal <- apply(results_matrix_FE_SSEL_bal, 2, mean)
#sd_coeffs_FE_SSEL_bal <- apply(results_matrix_FE_SSEL_bal , 2, sd)
#bias_FE_SSEL_bal <- mean_coeffs_FE_SSEL_bal - true_coeffs_FE
#rmse_FE_SSEL_bal <- sqrt(apply((results_matrix_FE_SSEL_bal - true_coeffs_FE)^2, 2, mean))
#
##----Print the results---#
#print(mean_coeffs_FE_SSEL_bal)
#print(sd_coeffs_FE_SSEL_bal)
#print(bias_FE_SSEL_bal)
#print(rmse_FE_SSEL_bal)
#
#
############################################
###---------------------OLS--------------------######
############################################
#model_results_OLS_SSEL_bal <- vector("list", n_simulations)
#for (i in 1:n_simulations) {
#  data <- data.frame(
#    id = rep(1:n, each = 2),   
#    time = rep(0:1, times = n),  
...
#  # FE regression on balanced data
#  balanced_dataAT <- balanced_dataAT %>% mutate(did = d * time)
#  didOLS_SSEL_bal <- lm(Y ~ d + time + did, data = balanced_dataAT)
#  model_results_OLS_SSEL_bal[[i]] <- coef(didOLS_SSEL_bal)
#}
#
##----results---#
#results_matrix_OLS_SSEL_bal <- do.call(rbind, model_results_OLS_SSEL_bal)
#mean_coeffs_OLS_SSEL_bal <- apply(results_matrix_OLS_SSEL_bal, 2, mean)
#sd_coeffs_OLS_SSEL_bal <- apply(results_matrix_OLS_SSEL_bal , 2, sd)
#bias_OLS_SSEL_bal <- mean_coeffs_OLS_SSEL_bal - true_coeffs_OLS
#rmse_OLS_SSEL_bal <- sqrt(apply((results_matrix_OLS_SSEL_bal - true_coeffs_OLS)^2, 2, mean))
#
##----Print the results---#
#print(mean_coeffs_OLS_SSEL_bal)
#print(sd_coeffs_OLS_SSEL_bal)
#print(bias_OLS_SSEL_bal)
#print(rmse_OLS_SSEL_bal)
#
#
#